...
const handleSubmit=async(e)=>{
      ...
      try{
         const response =await axios.post("http://localhost:8000/api/v1/college-data/check-college-details",formData);
         if(response.data.success){
          alert("Deatils Verified Succefully from College!");
          navigate(`/nextStep/${studentId}`)
         }
      }
      catch(err){
        console.error(err);
